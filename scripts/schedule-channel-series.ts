...
async function main(): Promise<void> {
  ...
  const manifest = JSON.parse(await readFile(path.join(root, 'assets', 'channel', 'manifest.json'), 'utf8')) as {
    assets: Array<{ contentKey: string; scheduledAt: string; caption: string; png: string }>;
  };
  ...
  for (const asset of manifest.assets) {
    ...
    if (previous) {
      if (previous.text !== asset.caption || previous.scheduledAt !== asset.scheduledAt || previous.photoSource?.kind !== 'local_path' || previous.photoSource.value !== localPath) {
        throw new Error(`Existing content ${asset.contentKey} does not match the verified manifest.`);
      }
      scheduled.push({ id: previous.id, contentKey: asset.contentKey, scheduledAt: previous.scheduledAt!, image: asset.png });
      continue;
    }
    ...
    const result = await store.schedule(draft.id, asset.scheduledAt, 0);
