...
export class JsonLeadStore {
  ...
  async toCsv(leads?: Lead[]): Promise<string> {
    ...
    const headers: (keyof Lead)[] = ['id','createdAt','updatedAt','telegramId','username','firstName','lastName','fullName','phone','city','age','workStatus','experience','goal','paymentOption','status','source','campaignId','intent','lastMessage','operatorNote','nextFollowUp','paymentStatus','preferredTime','notes'];
